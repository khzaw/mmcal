...
function reducer(state: CalendarState, action: Action): CalendarState {
  switch (action.type) {
    ...
    case "TODAY": {
      ...
      return { ...state, year: y, month: m, day: d, selectedJdn: jdn, selectedDay: day }
    }
    case "GO_TO_DATE": {
...
export function useCalendarState() {
  ...
  const m = now.getMonth() + 1
  const d = now.getDate()
  const todayJdn = Math.round(w2j(y, m, d))
  const todayInfo = getDayInfo(todayJdn)
  ...
  const [state, dispatch] = useReducer(reducer, {
    ...
    selectedDay: todayInfo,
    view: "month" as ViewMode,
  })

  return { state, dispatch, todayJdn, todayInfo }
}
